...
<style>
    ...
    /* 針對特定按鈕的 key 值 */
    .stButton button[data-testid="baseButton-secondary"]:nth-of-type(1),
    .stButton button[data-testid="baseButton-secondary"]:nth-of-type(2),
    .stButton button[data-testid="baseButton-secondary"]:nth-of-type(3) {
        background-color: transparent !important;
        color: #28a745 !important;
        border: 1px solid #28a745 !important;
    }
    
    .stButton button[data-testid="baseButton-secondary"]:nth-of-type(1):hover,
    .stButton button[data-testid="baseButton-secondary"]:nth-of-type(2):hover,
    .stButton button[data-testid="baseButton-secondary"]:nth-of-type(3):hover {
        background-color: rgba(40, 167, 69, 0.1) !important;
        color: #218838 !important;
    }
    
    /* 文件上傳器 - Browse files 按鈕無底色無框 */
    ...
    /* 文件上傳器 - 刪除按鈕 (叉叉) 無底色無框 */
    .stFileUploader button[title*="Remove"],
    .stFileUploader button[title*="remove"],
    .stFileUploader button[data-testid*="remove"] {
        background-color: transparent !important;
        color: #28a745 !important;
        border: none !important;
    }
    
    .stFileUploader button[title*="Remove"]:hover,
    .stFileUploader button[title*="remove"]:hover,
    .stFileUploader button[data-testid*="remove"]:hover {
        background-color: rgba(40, 167, 69, 0.1) !important;
        color: #218838 !important;
        border: none !important;
    }
    ...
    /* 徹底移除三個特定按鈕的綠色底色 */
    .stButton button:contains("🔄 全選"),
    .stButton button:contains("📊 核心欄位"), 
    .stButton button:contains("🎯 預測欄位"),
    ...
    .stButton button:contains("🔄 全選"):hover,
    .stButton button:contains("📊 核心欄位"):hover,
    .stButton button:contains("🎯 預測欄位"):hover,
    ...
    /* 覆寫所有可能的綠色按鈕樣式 */
    .stButton > button[style*="background-color: rgb(40, 167, 69)"] {
        background-color: transparent !important;
        color: #28a745 !important;
        border: 1px solid #28a745 !important;
    }
    ...
    button[style*="background-color: rgb(255, 75, 75)"]:hover,
    button[style*="background-color: #ff4b4b"]:hover,
    ...
    /* 日曆中選中的日期 */
    .stDateInput div[data-baseweb="calendar"] button[aria-selected="true"] {
        background-color: #28a745 !important;
        color: white !important;
    }
    
    /* 日曆中今天的日期圓圈 */
    .stDateInput div[data-baseweb="calendar"] button[aria-label*="today"] {
        border-color: #28a745 !important;
    }
    ...
    /* 選中狀態 - 內部圓點變綠色 */
    .stRadio div[role="radiogroup"] label div div[data-checked="true"],
    ...
    select[style*="border-color: #dc3545"] {
        border-color: #28a745 !important;
    }
    ...
    div[style*="border-color: #dc3545"] {
        color: #28a745 !important;
        border-color: #28a745 !important;
    }
    
    /* 徹底清除 Streamlit 預設的紅色樣式 */
    .stAlert, .stException, .stError > div {
        background-color: #e8f5e8 !important;
        color: #1e5631 !important;
        border-color: #28a745 !important;
    }
...
if st.session_state.prediction_data is not None:
    ...
    with col3:
        # 高轉換機率用戶 - 網投
        high_online_users = len(df[df['Online_conversion_prob'] >= 0.5])
        online_rate = high_online_users / len(df) * 100
        st.metric("💻 高轉換機率 (≥0.5) 用戶-網投", f"{high_online_users} ({online_rate:.1f}%)")
    with col4:
        # 高轉換機率用戶 - 預約O2O
        high_o2o_users = len(df[df['O2O_reservation_prob'] >= 0.5])
        o2o_rate = high_o2o_users / len(df) * 100
        st.metric("🤝 高轉換機率 (≥0.5) 用戶-預約O2O", f"{high_o2o_users} ({o2o_rate:.1f}%)")
    ...
    with col3:
        st.metric("💻 高轉換機率 (≥0.5) 用戶-網投", "---")
    with col4:
        st.metric("🤝 高轉換機率 (≥0.5) 用戶-預約O2O", "---")
...
if st.session_state.prediction_data is not None:
    ...
    with st.expander("🔍 設定篩選條件", expanded=True):
        ...
        with col1:
            ...
            history_steps = st.selectbox("最近幾Ｎ內", options=list(range(2, 10)), index=6)
